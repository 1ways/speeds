...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var worksList = document.querySelector('.works__items'),
      worksListHeight = worksList.clientHeight,
      showBtn = document.querySelector('.works__more');

  if (worksList.children.length > 8) {
    showBtn.classList.add('active');
    worksList.style.maxHeight = '540px';
    showBtn.addEventListener('click', function (e) {
      if (showBtn.textContent == 'показати все') {
        showBtn.textContent = 'сховати';
        showWorks();
      } else {
        showBtn.textContent = 'показати все';
        closeWorks();
      }
    });
  }
  ...
  function showWorks() {
    worksList.style.maxHeight = worksListHeight + 'px';
  }
